...
class real_sparse_solver_cuda_helpers {
 ...
 public:
  explicit real_sparse_solver_cuda_helpers()
      : solver_handle_{NULL}, mat_desc_{NULL} {}

  ~real_sparse_solver_cuda_helpers() { cusolverSpDestroy(solver_handle_); }
  ...
  inline void initialize() {
    ...
    LSS_ASSERT(cusparseSetMatIndexBase(mat_desc_, CUSPARSE_INDEX_BASE_ZERO) ==
                   CUSOLVER_STATUS_SUCCESS,
               " Failure while setting indexing style");
    LSS_ASSERT(cusparseSetMatType(mat_desc_, CUSPARSE_MATRIX_TYPE_GENERAL) ==
                   CUSOLVER_STATUS_SUCCESS,
  ...
  inline cusparseMatDescr_t const& get_matrix_descriptor() { return mat_desc_; }
};
...
class real_dense_solver_cuda_helpers {
 ...
 public:
  explicit real_dense_solver_cuda_helpers()
      : solver_handle_{NULL}, cublas_handle_{NULL} {}
  ...
  inline void initialize() {
    ...
    LSS_ASSERT((cublasCreate_v2(&cublas_handle_) == CUBLAS_STATUS_SUCCESS),
  ...
  inline cublasHandle_t const& get_cublas_handle() { return cublas_handle_; }
};
