...
class RealSparseSolverCUDAHelpers {
 ...
 public:
  ...
  ~RealSparseSolverCUDAHelpers() { cusolverSpDestroy(solverHandle_); }
  ...
  inline void initialize() {
    ...
    LSS_ASSERT(cusparseSetMatIndexBase(matDesc_, CUSPARSE_INDEX_BASE_ZERO) ==
                   CUSOLVER_STATUS_SUCCESS,
               " Failure while setting indexing style");
    LSS_ASSERT(cusparseSetMatType(matDesc_, CUSPARSE_MATRIX_TYPE_GENERAL) ==
                   CUSOLVER_STATUS_SUCCESS,
...
class RealDenseSolverCUDAHelpers {
 ...
 public:
  ...
  inline cusolverDnHandle_t const& getDenseSolverHandle() {
    return solverHandle_;
  }
