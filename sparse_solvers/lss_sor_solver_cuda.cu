#include "lss_sor_solver_cuda.h"
#include "lss_sor_solver_kernels.h"

namespace lss_sor_solver {

template <>
void sor_solver_core<double>::operator()(
  ...
  sor_kernel<double><<<threads_per_block, blocks_per_grid>>>(
      ...
      diag_data_ptr, row_start_idx_data_ptr, row_end_idx_data_ptr, omega,
      sol_data_ptr, next_sol_data_ptr, errors_data_ptr);
}

template <>
void sor_solver_core<float>::operator()(
