...
template <typename fp_type> class sor_cuda_boundary
{
  ...
  public:
    ...
    explicit sor_cuda_boundary(const boundary_1d_ptr<fp_type> &first, const boundary_1d_ptr<fp_type> &second,
...
template <typename fp_type> void sor_cuda_boundary<fp_type>::initialise(fp_type time)
{
...
template <typename fp_type> void sor_cuda_boundary<fp_type>::finalise(fp_type time)
{
...
template <typename fp_type> const fp_type sor_cuda_boundary<fp_type>::upper_boundary(fp_type time)
{
    fp_type ret{};
    if (auto ptr = std::dynamic_pointer_cast<dirichlet_boundary_1d<fp_type>>(second_))
    {
...
template <typename fp_type> const fp_type sor_cuda_boundary<fp_type>::lower_boundary(fp_type time)
{
    fp_type ret{};
    if (auto ptr = std::dynamic_pointer_cast<dirichlet_boundary_1d<fp_type>>(first_))
    {
