...
template <typename fp_type> class sor_boundary
{
  ...
  public:
    ...
    explicit sor_boundary(const std::size_t discretization_size, const fp_type &space_step)
...
template <typename fp_type> using sor_boundary_ptr = sptr_t<sor_boundary<fp_type>>;
