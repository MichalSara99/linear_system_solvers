#if !defined(_LSS_1D_GENERAL_SVC_HESTON_EQUATION_BUILDER_HPP_)
#define _LSS_1D_GENERAL_SVC_HESTON_EQUATION_BUILDER_HPP_
...
class general_svc_heston_equation_builder
{
  private:
  ...
  public:
    general_svc_heston_equation_builder &heat_data_config(const heat_data_config_2d_ptr<fp_type> &heat_data_config)
    {
        heat_data_config_ = heat_data_config;
        return *this;
    }

    general_svc_heston_equation_builder &discretization_config(
    ...
    general_svc_heston_equation_builder &vertical_upper_boundary(const boundary_2d_ptr<fp_type> &boundary_ptr)
    {
        vertical_upper_boundary_ = boundary_ptr;
        return *this;
    }

    general_svc_heston_equation_builder &horizontal_boundary(const boundary_2d_pair<fp_type> &boundary_pair)
    {
        horizontal_boundary_pair_ = boundary_pair;
        return *this;
    }

    general_svc_heston_equation_builder &splitting_method_config(
    ...
    general_svc_heston_equation_builder &solver_config(const heat_implicit_solver_config_ptr &solver_config)
    {
        solver_config_ = solver_config;
        return *this;
    }

    general_svc_heston_equation_builder &solver_config_details(
    ...
    sptr_t<general_svc_heston_equation<fp_type, container, allocator>> build()
    {
...
#endif ///_LSS_1D_GENERAL_SVC_HESTON_EQUATION_BUILDER_HPP_
