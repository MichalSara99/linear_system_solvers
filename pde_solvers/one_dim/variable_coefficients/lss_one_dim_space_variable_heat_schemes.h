#pragma once
#if !defined(_LSS_ONE_DIM_SPACE_VARIABLE_HEAT_SCHEMES)
#define _LSS_ONE_DIM_SPACE_VARIABLE_HEAT_SCHEMES
...
namespace lss_one_dim_space_variable_heat_schemes {
	...
	template<typename T>
	using SchemeCoefficientHolder = std::tuple<std::function<T(T)>, std::function<T(T)>, std::function<T(T)>, T, T>;

	template<typename T>
	using SchemeFunction = std::function<void(SchemeCoefficientHolder<T> const&, std::vector<T> const &, std::vector<T> const &,
		std::vector<T> const &, std::vector<T>&)>;

	template<typename T>
	using SchemeFunctionCUDA = std::function<void(SchemeCoefficientHolder<T> const&,
	...
	public:
		...
		static SchemeFunction<T> const getScheme(ImplicitPDESchemes scheme) {
			...
			if (scheme == ImplicitPDESchemes::Euler)
				...
				theta = 0.5;
			auto schemeFun = [=](SchemeCoefficientHolder<T> const& coeffs,
...
template<typename T>
bool lss_one_dim_space_variable_heat_schemes::ExplicitHeatEulerScheme<T>::isStable()const {
...
template<typename T>
void lss_one_dim_space_variable_heat_schemes::ExplicitHeatEulerScheme<T>::operator()(std::pair<T, T> const &dirichletBCPair,
...
template<typename T>
void lss_one_dim_space_variable_heat_schemes::ExplicitHeatEulerScheme<T>::operator()(std::pair<T, T> const &leftRobinBCPair,
	std::pair<T, T> const &rightRobinBCPair,
	std::vector<T> &solution)const {
	LSS_ASSERT(solution.size() > 0,
		"The input solution container must be initialized.");
...
template<typename T>
void lss_one_dim_space_variable_heat_schemes::ADEHeatBakaratClarkScheme<T>::operator()(std::pair<T, T> const &dirichletBCPair,
	std::vector<T> &solution)const {
	LSS_ASSERT(solution.size() > 0,
		"The input solution container must be initialized.");
	LSS_ASSERT(solution.size() == initialCondition_.size(),
...
template<typename T>
void lss_one_dim_space_variable_heat_schemes::ADEHeatBakaratClarkScheme<T>::operator()(std::pair<T, T> const &leftRobinBCPair,
	std::pair<T, T> const &rightRobinBCPair,
	std::vector<T> &solution)const {
...
template<typename T>
void lss_one_dim_space_variable_heat_schemes::ADEHeatSaulyevScheme<T>::operator()(std::pair<T, T> const &dirichletBCPair,
	std::vector<T> &solution)const {
	LSS_ASSERT(solution.size() > 0,
		"The input solution container must be initialized.");
	LSS_ASSERT(solution.size() == initialCondition_.size(),
...
template<typename T>
void lss_one_dim_space_variable_heat_schemes::ADEHeatSaulyevScheme<T>::operator()(std::pair<T, T> const &leftRobinBCPair,
	std::pair<T, T> const &rightRobinBCPair,
	std::vector<T> &solution)const {
	throw new std::exception("Not available.");
}
...
#endif //_LSS_ONE_DIM_SPACE_VARIABLE_HEAT_SCHEMES