...
namespace lss_one_dim_space_variable_heat_explicit_schemes {
	...
	template<typename T>
	using SchemeCoefficientHolder = std::tuple<std::function<T(T)>, std::function<T(T)>, std::function<T(T)>>;
	...
	public:
		...
		explicit ExplicitHeatEulerScheme(T spaceStart,
			...
			:Explicit1DHeatSchemeBase<T, SchemeCoefficientHolder<T>>(spaceStart,
																	terminalTime,
																	deltas,
																	coeffs,
																	initialCondition,
...
template<typename T>
bool lss_one_dim_space_variable_heat_explicit_schemes::ExplicitHeatEulerScheme<T>::isStable()const {
	...
	for (std::size_t i = 0; i < spaceSize; ++i) {
		if (c(i*h) > 0.0)
			return false;
		if ((2.0*lambda*a(i*h) - k * c(i*h)) > 1.0)
			return false;
		if (((gamma*std::abs(b(i*h)))*(gamma*std::abs(b(i*h)))) > (2.0*lambda*a(i*h)))
			return false;
	}
	return true;
}
...
template<typename T>
void lss_one_dim_space_variable_heat_explicit_schemes::ExplicitHeatEulerScheme<T>::operator()(std::pair<T, T> const &dirichletBCPair,
	...
	if (!isSourceSet_) {
		...
		while (time <= terminalTime_) {
			...
			solution[solution.size() - 1] = right;
			for (std::size_t t = 1; t < spaceSize - 1; ++t) {
				solution[t] = solution[t] = (1.0 - 2.0*B(t*h)) * prevSol[t] +
					D(t*h) * prevSol[t + 1] + A(t*h) * prevSol[t - 1] + 
					k * sourceCurr[t];
			}
...
template<typename T>
void lss_one_dim_space_variable_heat_explicit_schemes::ExplicitHeatEulerScheme<T>::operator()(std::pair<T, T> const &leftRobinBCPair,
	std::pair<T, T> const &rightRobinBCPair,
	std::vector<T> &solution)const {
	LSS_ASSERT(solution.size() > 0,
		"The input solution container must be initialized.");
	...
	if (!isSourceSet_) {
		...
		while (time <= terminalTime_) {
			...
			solution[spaceSize - 1] = (A((spaceSize - 1)*h) + (D((spaceSize - 1)*h) * rightLin))*prevSol[spaceSize - 2] +
				(1.0 - 2.0*B((spaceSize - 1)*h)) * prevSol[spaceSize - 1] + D((spaceSize - 1)*h) * rightConst;
			for (std::size_t t = 1; t < spaceSize - 1; ++t) {
				solution[t] = (1.0 - 2.0*B(t*h)) * prevSol[t] +
					D(t*h) * prevSol[t + 1] + A(t*h) * prevSol[t - 1];
			}
...
template<typename T>
void lss_one_dim_space_variable_heat_explicit_schemes::ADEHeatBakaratClarkScheme<T>::operator()(std::pair<T, T> const &dirichletBCPair,
	std::vector<T> &solution)const {
	LSS_ASSERT(solution.size() > 0,
		"The input solution container must be initialized.");
	LSS_ASSERT(solution.size() == initialCondition_.size(),
	...
	auto upSweep = [=](std::vector<T>& upComponent, std::vector<T> const &rhs, T rhsCoeff) {
		for (std::size_t t = 1; t < spaceSize - 1; ++t) {
			upComponent[t] = b(t*h) * upComponent[t] + d(t*h) * upComponent[t + 1] +
	...
	auto downSweep = [=](std::vector<T>& downComponent, std::vector<T> const &rhs, T rhsCoeff) {
		for (std::size_t t = spaceSize - 2; t >= 1; --t) {
			downComponent[t] = b(t*h) * downComponent[t] + d(t*h) * downComponent[t + 1] +
...
template<typename T>
void lss_one_dim_space_variable_heat_explicit_schemes::ADEHeatBakaratClarkScheme<T>::operator()(std::pair<T, T> const &leftRobinBCPair,
	std::pair<T, T> const &rightRobinBCPair,
	std::vector<T> &solution)const {
...
template<typename T>
void lss_one_dim_space_variable_heat_explicit_schemes::ADEHeatSaulyevScheme<T>::operator()(std::pair<T, T> const &dirichletBCPair,
	std::vector<T> &solution)const {
	LSS_ASSERT(solution.size() > 0,
		"The input solution container must be initialized.");
	LSS_ASSERT(solution.size() == initialCondition_.size(),
...
template<typename T>
void lss_one_dim_space_variable_heat_explicit_schemes::ADEHeatSaulyevScheme<T>::operator()(std::pair<T, T> const &leftRobinBCPair,
	std::pair<T, T> const &rightRobinBCPair,
	std::vector<T> &solution)const {
	throw new std::exception("Not available.");
}
