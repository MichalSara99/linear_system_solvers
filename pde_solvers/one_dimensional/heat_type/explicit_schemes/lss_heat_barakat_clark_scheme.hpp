...
class heat_barakat_clark_time_loop
{
...
template <typename fp_type, template <typename, typename> typename container, typename allocator>
template <typename solver>
void heat_barakat_clark_time_loop<fp_type, container, allocator>::run(
...
template <typename fp_type, template <typename, typename> typename container, typename allocator>
template <typename solver>
void heat_barakat_clark_time_loop<fp_type, container, allocator>::run_with_stepping(
    solver const &solver_ptr, boundary_1d_pair<fp_type> const &boundary_pair, range<fp_type> const &time_range,
    std::size_t const &last_time_idx, fp_type const time_step, traverse_direction_enum const &traverse_dir,
    container_t &solution, container_2d_t &solutions)
{
...
{
    typedef heat_barakat_clark_time_loop<fp_type, container, allocator> loop;
    typedef discretization<dimension_enum::One, fp_type, container, allocator> d_1d;
    typedef container<fp_type, allocator> container_t;

  private:
    heat_barakat_clark_coefficients_ptr<fp_type> bc_coeffs_;
    ...
    void initialize(general_heat_equation_coefficients_ptr<fp_type> const &coefficients)
    {
        ...
        bc_coeffs_ = std::make_shared<heat_barakat_clark_coefficients<fp_type>>(coefficients);
    }

    explicit heat_barakat_clark_scheme() = delete;

  public:
    heat_barakat_clark_scheme(general_heat_equation_coefficients_ptr<fp_type> const &coefficients,
