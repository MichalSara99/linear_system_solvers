...
class implicit_heston_scheme_hv
{
  ...
  public:
    ...
    static void get_intermediate_3(general_svc_heston_equation_coefficients_ptr<fp_type> const &cfs,
                                   grid_config_2d_ptr<fp_type> const &grid_cfg, rcontainer_2d_t const &input,
                                   rcontainer_2d_t const &inhom_input, rcontainer_2d_t const &inhom_input_next,
                                   fp_type const &time, rcontainer_2d_t &solution)
    {
        const fp_type one = static_cast<fp_type>(1.0);
        ...
        {
            x = grid_2d<fp_type>::value_1(grid_cfg, r);
            for (std::size_t c = 1; c < cols; ++c)
            {
                y = grid_2d<fp_type>::value_2(grid_cfg, c);
                val = (-theta * M(x, y, one) * inhom_input(r - 1, c)) +
                      ((one + theta * Z(x, y, one)) * inhom_input(r, c)) -
                      (theta * P(x, y, one) * inhom_input(r + 1, c)) + (theta * M(x, y, one) * input(r - 1, c)) -
                      (theta * Z(x, y, one) * input(r, c)) + (theta * P(x, y, one) * input(r + 1, c)) +
                      (zeta * gamma * C(x, y) *
                       ((inhom_input_next(r + 1, c + 1) - input(r + 1, c + 1)) -
                        (inhom_input_next(r + 1, c - 1) - input(r + 1, c - 1)) -
                        (inhom_input_next(r - 1, c + 1) - input(r - 1, c + 1)) +
                        (inhom_input_next(r - 1, c - 1) - input(r - 1, c - 1)))) +
                      (zeta * (M(x, y, one) * (inhom_input_next(r - 1, c) - input(r - 1, c)) -
                               (W(x, y, one) + Z(x, y, one)) * (inhom_input_next(r, c) - input(r, c)) +
                               P(x, y, one) * (inhom_input_next(r + 1, c) - input(r + 1, c)) +
                               M_tilde(x, y, one) * (inhom_input_next(r, c - 1) - input(r, c - 1)) +
                               P_tilde(x, y, one) * (inhom_input_next(r, c + 1) - input(r, c + 1))));
                solution(r, c, val);
            }
        }
    }
...
class heat_hundsdorfer_verwer_method : public heat_splitting_method<fp_type, container, allocator>
{
  ...
  public:
    explicit heat_hundsdorfer_verwer_method(solver const &solvery_ptr, solver const &solveru_ptr,
    ...
    container_2d<by_enum::Row, fp_type, container, allocator> &solution)
{
    ...
    rcontainer_2d_t inter_solution_3(coefficients_->space_size_x_, coefficients_->space_size_y_, fp_type{});
    heston_scheme::get_intermediate_3(coefficients_, grid_cfg_, prev_solution, inter_solution_1, inter_solution_2, time,
                                      inter_solution_3);

    // 2D container for intermediate solution Y_4:
    ccontainer_2d_t inter_solution_4(coefficients_->space_size_x_, coefficients_->space_size_y_, fp_type{});
    ...
    // get the right-hand side of the scheme:
    auto scheme_y4 = heston_scheme::get_intermediate_4();
    for (std::size_t j = 1; j < coefficients_->space_size_y_ - 1; ++j)
    {
        y = grid_2d<fp_type>::value_2(grid_cfg_, j);
        split_0(y, low, diag, high);
        scheme_y4(coefficients_, grid_cfg_, j, y, inter_solution_2, inter_solution_3, curr_source, time, rhs);
        solvery_ptr_->set_diagonals(low, diag, high);
        solvery_ptr_->set_rhs(rhs);
        solvery_ptr_->solve(horizontal_boundary_pair, solution_v, time, y);
    ...
    for (std::size_t i = 1; i < coefficients_->space_size_x_ - 1; ++i)
    {
        x = grid_2d<fp_type>::value_1(grid_cfg_, i);
        split_1(x, low, diag, high);
        scheme_u(coefficients_, grid_cfg_, i, x, inter_solution_2, inter_solution_4, curr_source, time, rhs);
        solveru_ptr_->set_diagonals(low, diag, high);
        solveru_ptr_->set_rhs(rhs);
        solveru_ptr_->solve(vertical_boundary_pair, solution_v, time, x);
