...
/**
 * heston_euler_cuda_time_loop object
 */
template <typename fp_type, template <typename, typename> typename container, typename allocator>
class heston_euler_cuda_time_loop
{
...
template <typename fp_type, template <typename, typename> typename container, typename allocator>
template <typename solver, typename boundary_solver>
void heston_euler_cuda_time_loop<fp_type, container, allocator>::run(
...
{
    typedef heston_euler_cuda_time_loop<fp_type, container, allocator> loop;
  ...
  public:
    heston_euler_cuda_scheme(general_heston_equation_coefficients_ptr<fp_type> const &coefficients,
                             boundary_2d_ptr<fp_type> const &vertical_upper_boundary_ptr,
                             boundary_2d_pair<fp_type> const &horizontal_boundary_pair,
                    ...
                    traverse_direction_enum traverse_dir)
    {
        ...
        auto const &solver_method_ptr =
            std::make_shared<heston_euler_cuda_solver_method<fp_type, container, allocator>>(euler_coeffs_, grid_cfg_,
                                                                                             is_heat_sourse_set);
