...
class container_3d
{
  private:
    ...
    std::size_t columns_;
    std::size_t layers_;
    std::vector<container_2d<by, fp_type, container, allocator>> rdata_;
  ...
  public:
    ...
    typedef container_2d<by, fp_type, container, allocator> element_type;
        ...
        : rows_{rows}, columns_{columns}, layers_{layers}
    {
        ...
        {
            rdata_.emplace_back(container_2d<by, fp_type, container, allocator>(rows, columns));
        }
    }
        ...
        : rows_{rows}, columns_{columns}, layers_{layers}
    {
        for (std::size_t l = 0; l < layers; ++l)
        {
            rdata_.emplace_back(container_2d<by, fp_type, container, allocator>(rows, columns, value));
        }
    }
    ...
    container_2d<by, fp_type, container, allocator> operator()(std::size_t lay_idx) const
    {
        LSS_ASSERT(lay_idx < layers_, "Outside of layer range");
    ...
    // return  container_2d at potision (lay_idx)
    container_2d<by, fp_type, container, allocator> at(std::size_t lay_idx) const
    {
        return operator()(lay_idx);
    }

    // place container_2d at row position (row_idx)
    void operator()(std::size_t lay_idx, container_2d<by, fp_type, container, allocator> const &container2d)
    {
        LSS_ASSERT(lay_idx < layers_, "Outside of layer range");
