...
template <typename fp_type, template <typename, typename> typename container, typename allocator> class container_3d
{
  private:
    std::size_t rows_;
    std::size_t columns_;
    std::size_t layers_;
    std::vector<container_2d<fp_type, container, allocator>> data_;
  ...
  public:
    ...
    typedef container_2d<fp_type, container, allocator> element_type;
        ...
        : rows_{rows}, columns_{columns}, layers_{layers}
    {
        ...
        {
            data_.emplace_back(container_2d<fp_type, container, allocator>(rows, columns));
        }
    }

    explicit container_3d(std::size_t rows, std::size_t columns, std::size_t layers, fp_type value)
        : rows_{rows}, columns_{columns}, layers_{layers}
    {
        for (std::size_t l = 0; l < layers; ++l)
        {
            data_.emplace_back(container_2d<fp_type, container, allocator>(rows, columns, value));
        }
    }
    ...
    fp_type operator()(std::size_t row_idx, std::size_t col_idx, std::size_t lay_idx) const
    {
    ...
    container_2d<fp_type, container, allocator> operator()(std::size_t lay_idx) const
    {
        LSS_ASSERT(lay_idx < layers_, "Outside of layer range");
    ...
    fp_type at(std::size_t row_idx, std::size_t col_idx, std::size_t lay_idx) const
    {
        return operator()(row_idx, col_idx, lay_idx);
    }

    // return  container_2d at potision (lay_idx)
    container_2d<fp_type, container, allocator> at(std::size_t lay_idx) const
    {
        return operator()(lay_idx);
    }

    // place container_2d at row position (row_idx)
    void operator()(std::size_t lay_idx, container_2d<fp_type, container, allocator> const &container2d)
    {
        LSS_ASSERT(lay_idx < layers_, "Outside of layer range");
        LSS_ASSERT(container2d.rows() == rows_, "Outside of row range");
        LSS_ASSERT(container2d.columns() == columns_, "Outside of column range");
        data_[lay_idx] = std::move(container2d);
    }
    ...
    void operator()(std::size_t row_idx, std::size_t col_idx, std::size_t lay_idx, fp_type value)
    {
        LSS_ASSERT(row_idx < rows_, "Outside of row range");
        LSS_ASSERT(col_idx < columns_, "Outside of column range");
        LSS_ASSERT(lay_idx < layers_, "Outside of layer range");
