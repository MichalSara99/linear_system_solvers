...
template <typename T> void test_impl_heston_equation_builder_t()
{
...
void test_impl_heston_equation_builder()
{
    test_impl_heston_equation_builder_t<float>();
    test_impl_heston_equation_builder_t<double>();
}

template <typename T> void test_expl_heston_equation_builder_t()
{
    using lss_boundary::dirichlet_boundary_2d_builder;
    using lss_boundary::neumann_boundary_2d_builder;
    ...
    using lss_pde_solvers::two_dimensional::explicit_solvers::general_heston_equation_builder;
    using lss_print::print;
    using lss_utility::pi;
    using lss_utility::range_builder;
    ...
    auto const &pde_solver = general_heston_equation_builder<T, std::vector, std::allocator<T>>()
                                 ...
                                 .horizontal_boundary(horizontal_boundary_pair)
                                 .grid_hints(grid_config_hints_ptr)
                                 .solver_config(host_expl_bwd_euler_solver_config_ptr)
                                 .build();
    ...
    // print approx only:
    std::stringstream ssa;
    std::string name = "ExplHestonEquationEulerHostFromBuilder_";
    ssa << "outputs/" << name << typeid(T).name() << ".txt";
    std::string file_name_approx{ssa.str()};
    std::ofstream approx(file_name_approx);
    print(discretization_ptr, grid_config_hints_ptr, solution, approx);
...
void test_expl_heston_equation_builder()
{
    test_expl_heston_equation_builder_t<float>();
    test_expl_heston_equation_builder_t<double>();
}
