...
namespace lss_pde_cuda_kernels {
	...
	template<typename T>
	__global__
	void fillRobinBC1D(T *solution, T lambda, T leftLinear, T leftConst, T rightLinear, T rightConst, unsigned long long size) {
		unsigned long long tid = blockIdx.x*blockDim.x + threadIdx.x;
		if (tid >= size)return;
		if (tid == 0)
			solution[tid] = lambda * (1.0 + leftLinear) * solution[tid + 1] +
			(1.0 - 2.0*lambda)*solution[tid] +
			lambda * leftConst;
		if (tid == (size - 1))
			solution[tid] = lambda * (1.0 + rightLinear) * solution[tid - 1] +
			(1.0 - 2.0*lambda)*solution[tid] +
			lambda * rightConst;
	}
